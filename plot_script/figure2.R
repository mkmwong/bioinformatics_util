...
plot_boxplot <- function(all_df, outpath, y.position=NULL, ylab = NULL,rotate = FALSE, 
                        de = "pdf", w = 6, h = 3.5, dp = 600) {
stat_test <- all_df %>%
  group_by(Sig_type) %>%
...
if (rotate == TRUE) {
  a + theme(axis.text.x = element_text(angle = 30, hjust = 1))
}
...
ylabel <- "CPD log2(FC)"
plot_boxplot(all_df, "../../plot/CPD_Centromere.pdf", c(0.8,0.8,0.4), ylabel, TRUE)
...
all_df <- get_df(rbko, wt, "centromere", 1000000, "Pericentric")
plot_boxplot(all_df, "../../plot/H3K9me3_Centromere_new.pdf", c(1.4,1.2,0.5), ylabel, TRUE)
...
all_df <- get_df(rbko, wt, "centromere", 1000000, "Pericentric")
plot_boxplot(all_df, "../../plot/H3K27me3_Centromere_new.pdf", c(1.5,1.2,0.5), ylabel, TRUE)
...
ylabel <- "CPD log2(FC)"
plot_boxplot(all_df, "../../plot/CPD_Telomere.pdf", c(0.7,0.6,0.3), ylabel, TRUE)
...
all_df <- get_df(rbko, wt, "telomere", 100000, "Subtelomeric")
all_df$group = factor(all_df$group, levels = c("Subtelomeric","Random"))
plot_boxplot(all_df, "../../plot/H3K9me3_Telomere_new.pdf", c(1.4,1.2,0.4), ylabel, TRUE)
...
all_df <- get_df(rbko, wt, "telomere", 100000, "Subtelomeric")
all_df$group = factor(all_df$group, levels = c("Subtelomeric","Random"))
plot_boxplot(all_df, "../../plot/H3K27me3_Telomere_new.pdf", c(1.6,1.5,0.5), ylabel, TRUE)
